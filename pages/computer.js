...
function iterationsToEscapeMandelbrotSet (cr, ci) {
  let iterations = 1
...
function iterationsToEscapeJuliaSet (zr, zi, kr, ki) {
  let iterations = 1
  // Local variables to store parts of the computation of z^2,
  // so that they cxan be reused for the computation of |z|.
  // They are zr^2 and zi^2.
...
function computeIterations (width, height, { x: offsetX, y: offsetY }, scale, type, { r: cr, i: ci } = {}) {
  ...
  for (let y = 0; y < height; ++y) {
    ...
    for (let x = 0; x < width; ++x) {
      iterations[row + x] = iterationsToEscapeSet(r, i, cr, ci)
...
function receiveRequest ({ data }) {
  ...
  const { width, height, offset, scale, type, k } = data
  const iterations = computeIterations(width, height, offset, scale, type, k)
